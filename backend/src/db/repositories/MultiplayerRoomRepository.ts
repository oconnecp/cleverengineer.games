...
export const insertNewMultiplayerRoom = async (board:string[][], totalPopularScore:number, gameIds:string[] = []): Promise<MultiplayerRoom> => {
  ...
  multiplayerRoom.gameIds = [...gameIds]; // Initialize with an empty array or the provided game IDs
...
export const findRoomByGameId = async (gameId: string): Promise<MultiplayerRoom[]> => {
  return await AppDataSource.getRepository(MultiplayerRoom)
